...
- Cyclic slot repetition: [Slot 0, Slot 1, Slot 2] with CRC-4 integrity verification
...
CRC4_POLY = 0x13  # x^4 + x + 1 (ITU-T G.704)
    ...
    try:
        ...
        for k in range(num_chunks):
            ...
            if k == num_chunks - 1 and (total_samples - end_idx) < chunk_samples:
                # Include remaining tail in last chunk
...
def extract_watermark(audio_path: str, target_sr=16000):
    ...
    try:
        ...
        # Collect detections per slot index: {0: [msgs], 1: [msgs], 2: [msgs]}
        ...
        # Determine bit consensus for a slot based on probability weights
        ...
        if resolved_s0 is not None and resolved_s1 is not None and resolved_s2 is not None:
            ...
            # Partial detection (e.g. short audio clip under 3s)
            return {
                'success': True,
                'detected': True,
                'partial': True,
